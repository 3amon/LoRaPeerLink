...
TEST_CASE("LoRaBasicLink packet framing", "[PacketFraming]") {
    ...
    SECTION("Packet format validation") {
        ...
        REQUIRE(rawLen == 11); // 5 (header) + 4 (payload) + 2 (CRC)
        
        // Validate packet structure
        REQUIRE(rawPacket[0] == 2);    // destination
        REQUIRE(rawPacket[1] == 1);    // source
        REQUIRE(rawPacket[2] == 0);    // sequence (first packet)
        REQUIRE(rawPacket[3] == 0);    // flags (no ACK requested)
        REQUIRE(rawPacket[4] == 4);    // payload length
        REQUIRE(rawPacket[5] == 't');  // payload start
        REQUIRE(rawPacket[6] == 'e');
        REQUIRE(rawPacket[7] == 's');
        REQUIRE(rawPacket[8] == 't');  // payload end
...
TEST_CASE("Packet size validation", "[PacketFraming]") {
    ...
    SECTION("LoRaBasicLink minimum packet size") {
        ...
        uint8_t rawPacket[256];
        int rawLen = radioB.receive(rawPacket, 256);
        REQUIRE(rawLen == 7); // 5 (header) + 0 (payload) + 2 (CRC)
    }
}
...
TEST_CASE("Flag handling in packets", "[PacketFraming]") {
    ...
    SECTION("LoRaBasicLink flag handling") {
        ...
        linkA.sendPacket(2, msg, 5, false);
        ...
        uint8_t msg2[] = {'a', 'c', 'k'};
        bool result = linkA.sendPacket(2, msg2, 3, true); // This will timeout since no ACK
...
TEST_CASE("Buffer boundary testing", "[PacketFraming]") {
    ...
    SECTION("Receiver buffer too small for packet") {
        ...
        REQUIRE(linkA.sendPacket(2, largePayload, 100) == true);
    ...
    SECTION("Zero-length buffer") {
        ...
        REQUIRE(linkA.sendPacket(2, payload, 4) == true);
