...
TEST_CASE("LoRaBackoffLink packet framing", "[BackoffPacketFraming]") {
    ...
    SECTION("Packet header structure") {
        ...
        REQUIRE(rawLen == 11); // 5 (PacketHeader) + 4 (payload) + 2 (CRC)
        
        // Validate PacketHeader structure
        REQUIRE(rawPacket[0] == 2);    // dst
        REQUIRE(rawPacket[1] == 1);    // src
        REQUIRE(rawPacket[2] == 0);    // seq (first packet)
        REQUIRE(rawPacket[3] == 0);    // flags (no ACK requested)
        REQUIRE(rawPacket[4] == 4);    // len
        REQUIRE(rawPacket[5] == 'd');  // payload
        REQUIRE(rawPacket[6] == 'a');
        REQUIRE(rawPacket[7] == 't');
...
TEST_CASE("LoRaBackoffLink minimum packet size", "[BackoffPacketFraming]") {
    ...
    uint8_t rawPacket[256];
    int rawLen = radioB.receive(rawPacket, 256);
    REQUIRE(rawLen == 7); // 5 (PacketHeader) + 0 (payload) + 2 (CRC)
}
